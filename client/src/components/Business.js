...
const Business = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const data = await axios.get(`http://localhost:4000/api/searchbycategory/business`)
        ...
        <div className="container">
            <h1>Business Blogs</h1>
            {
                blogs ?
                    blogs.map(item => {
                        return (
                            <div key={item._id} className="card flex-md-row mb-4 box-shadow h-md-250 my-4">
                                ...
                                <img className="card-img-right flex-auto d-none d-md-block" alt="Thumbnail [200x250]"
                                    style={{ width: 250, height: 250 }} src={item.picture} data-holder-rendered="true" />
                            </div>
                        )
                    })
                    : <h1>No Blog found</h1>
